 ...
 */
package l2.brick.gameserver.network;
...
import l2.brick.gameserver.model.CharSelectInfoPackage;
...
import l2.brick.util.EventData;
...
public final class L2GameClient extends MMOClient<MMOConnection<L2GameClient>> implements Runnable
{
	...
	public L2PcInstance loadCharFromDisk(int charslot)
	{
		...
		if (character != null && !character.getOfflineSuperMode())
		{
			...
			try
			{
				...
				if (player != null && !isDetached())
				{
					...
					if (!player.isInOlympiadMode()
							...
							&& !DM.isPlayerParticipant(player.getObjectId())
							&& !LMEvent.isPlayerParticipant(player.getObjectId())
							&& !TvTRoundEvent.isPlayerParticipant(player.getObjectId())
							&& !player.isInJail()
