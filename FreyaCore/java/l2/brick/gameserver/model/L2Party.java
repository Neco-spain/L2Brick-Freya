...
import l2.brick.gameserver.model.entity.DimensionalRift;
...
import l2.brick.gameserver.network.serverpackets.ExAskModifyPartyLooting;
import l2.brick.gameserver.network.serverpackets.ExCloseMPCC;
import l2.brick.gameserver.network.serverpackets.ExOpenMPCC;
import l2.brick.gameserver.network.serverpackets.ExPartyPetWindowAdd;
import l2.brick.gameserver.network.serverpackets.ExPartyPetWindowDelete;
import l2.brick.gameserver.network.serverpackets.ExSetPartyLooting;
import l2.brick.gameserver.network.serverpackets.L2GameServerPacket;
import l2.brick.gameserver.network.serverpackets.PartyMemberPosition;
import l2.brick.gameserver.network.serverpackets.PartySmallWindowAdd;
import l2.brick.gameserver.network.serverpackets.PartySmallWindowAll;
import l2.brick.gameserver.network.serverpackets.PartySmallWindowDelete;
import l2.brick.gameserver.network.serverpackets.PartySmallWindowDeleteAll;
...
public class L2Party
{
	 ...
	 * @see l2.brick.gameserver.model.actor.instance.L2PcInstance#isRequestExpired()
	 */
	...
	public void distributeItem(L2PcInstance player, L2Attackable.RewardItem item, boolean spoil, L2Attackable target)
	{
		...
		L2ItemInstance ditem = ItemTable.getInstance().createItem("Loot", item.getItemId(), item.getCount(), looter, target);

		looter.addItem(spoil?"Sweep":"Party", ditem.getItemId(), ditem.getCount(), player, true);
		...
		if (item.getCount() > 1)
		{
			...
			msg.addItemName(ditem.getItemId());
			msg.addItemNumber(ditem.getCount());
