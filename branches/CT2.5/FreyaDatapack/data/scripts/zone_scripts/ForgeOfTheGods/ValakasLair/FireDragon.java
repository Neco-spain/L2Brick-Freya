...
import static l2.brick.gameserver.ai.CtrlIntention.AI_INTENTION_IDLE;
...
import l2.brick.gameserver.network.serverpackets.SpecialCamera;
import l2.brick.gameserver.templates.StatsSet;
import l2.brick.gameserver.util.Util;
import l2.brick.util.Rnd;

public class FireDragon extends L2AttackableAIScript
{
	private int i_ai0 = 0;
	private int i_ai1 = 0;
	private int i_ai2 = 0;
	...
	// Boss: Valakas
	public FireDragon(int id, String name, String descr)
	{
		super(id, name, descr);
		int[] mob =
		{
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		if (npc != null)
		{
			...
			else if (event.equalsIgnoreCase("1002"))
			{
				...
				else if (npc.getCurrentHp() > ((npc.getMaxHp() * 1) / 4f))
				{
	...
	public String onSpawn(L2Npc npc, L2PcInstance player)
	{
		//((L2Character) obj).sendPacket(new SocialAction(npc, 3));
		player.sendPacket(new SocialAction(npc, 3));
	...
	public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isPet, L2Skill skill)
	{
		...
		else if (npc.getCurrentHp() > ((npc.getMaxHp() * 2) / 4f))
		{
			if (attacker == c_quest2)
			{
	...
	public static void main(String[] args)
	{
		...
		new FireDragon(-1, "FireDragon", "ai");
		if (Config.ENABLE_LOADING_INFO_FOR_SCRIPTS)
			_log.info("Loaded Forge Of The Gods: Valakas Lair - Fire Dragon");
	}
}