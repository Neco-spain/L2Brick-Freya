...
public class L2Spawn
{
	...
	public L2Spawn(L2NpcTemplate mobTemplate) throws SecurityException, ClassNotFoundException, NoSuchMethodException
	{
		...
		else if ((mobTemplate.npcId >= 31046)&&(mobTemplate.npcId <= 31053))
            implementationName = "L2SymbolMaker";

		// Create the generic constructor of L2NpcInstance managed by this L2Spawn
		Class<?>[] parameters = {int.class, Class.forName("l2.brick.gameserver.templates.L2NpcTemplate")};
		_constructor = Class.forName("l2.brick.gameserver.model.actor.instance." + implementationName + "Instance").getConstructor(parameters);
	}
	...
	public L2Npc spawnOnce()
	{
		return spawnOne(false);
	}
		...
		try
		{
			// Check if the L2Spawn is not a L2Pet or L2Minion or L2Decoy spawn
            		if (_template.type.equalsIgnoreCase("L2Pet")
            			...
            			|| _template.type.equalsIgnoreCase("L2FlyMinion")
	...
	public void setRespawnDelayRnd(int i)
	{
		if (i < 0)
		{
			_log.warning("respawn delay random is negative for spawn:" + this);
			i = 0;
		}
		_respawnDelayRnd = i * 1000;
	}
	...
	/**
	 * @param respawnDelayRnd the _respawnDelayRnd to set
	 */
	public void respawnDelayRnd(int respawnDelayRnd)
	{
		this._respawnDelayRnd = respawnDelayRnd;
	}

	/**
	 * @return the _respawnDelayRnd
	 */
	public int respawnDelayRnd()
	{
		return _respawnDelayRnd;
	}
}