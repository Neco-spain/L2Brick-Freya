...
import l2.brick.gameserver.model.actor.instance.L2ArtefactInstance;
...
import l2.brick.gameserver.skills.BaseStats;
...
public abstract class L2Skill implements IChanceSkillTrigger
{
					...
					if (player.getClan() != null)
					{
						...
						//synchronized (activeChar.getKnownList().getKnownObjects())
						{
							for (L2PcInstance obj : objs)
							{
								...
								if (!TvTEvent.checkForTvTSkill(player, obj, this) || !LMEvent.checkForLMSkill(player, obj, this) || !DM.checkForDMSkill(player, obj, this))
									continue;
	 ...
	 * If TvT event or TvT rounds is active - performing additional checks.
	 * 
	...
	public static final boolean checkForAreaOffensiveSkills(L2Character caster, L2Character target, L2Skill skill, boolean sourceInArena)
	{
		...
		if (player != null)
		{
			...
			if (targetPlayer != null)
			{
				...
				if (!TvTEvent.checkForTvTSkill(player, targetPlayer, skill) || !LMEvent.checkForLMSkill(player, targetPlayer, skill) || !DM.checkForDMSkill(player, targetPlayer, skill))
					return false;
		...
		for (EffectTemplate et : _effectTemplates)
		{
			...
			else if (et.icon && effector instanceof L2PcInstance)
			{
				...
				if (Config.SHOW_DAMAGE_MESSAGE_ON_CENTER_TOP_SCREEN)
					((L2PcInstance)effector).sendPacket(new ExShowScreenMessage(1,0,2,0,0,0,0,false,3000,1,"Your skill attack is not successful !"));
