 ...
 */
 ...
 /*
  * Community Board Legend by FFs
  */
package l2.brick.gameserver.communityboard;
...
import l2.brick.gameserver.GameServer;
...
public class RegionBBSManager extends BaseBBSManager
{
	...
	private void showOldCommunityPI(L2PcInstance activeChar, String name)
	{
		final StringBuilder htmlCode = StringUtil.startAppend(1000, "<html><body><br>"
				+ "<table border=0><tr><td FIXWIDTH=15></td><td align=center>L2JBrick Community Board<img src=\"sek.cbui355\" width=610 height=1></td></tr><tr><td FIXWIDTH=15></td><td>");
		...
		{
			final ShowBoard sb = new ShowBoard(StringUtil.concat("<html><body><br><br><center>No player with name ", name, "</center><br><br></body></html>"), "101");
			activeChar.sendPacket(sb);
			activeChar.sendPacket(new ShowBoard(null, "102"));
			activeChar.sendPacket(new ShowBoard(null, "103"));
		}
	...
	public void parsewrite(String ar1, String ar2, String ar3, String ar4, String ar5, L2PcInstance activeChar)
	{
		...
		if (ar1.equals("PM"))
		{
			...
			try
			{
				...
				{
					final SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.THE_PERSON_IS_IN_MESSAGE_REFUSAL_MODE);
		...
		{
			final ShowBoard sb = new ShowBoard(StringUtil.concat("<html><body><br><br><center>The command: ", ar1, " is not implemented yet</center><br><br></body></html>"), "101");
			activeChar.sendPacket(sb);
			activeChar.sendPacket(new ShowBoard(null, "102"));
			activeChar.sendPacket(new ShowBoard(null, "103"));
		}
	...
	private void addOnlinePlayer(L2PcInstance player)
	{
		...
		if (!added)
		{
			final FastList<L2PcInstance> temp = new FastList<L2PcInstance>();
	...
	private void writeCommunityPages()
	{
		...
		{
			final FastMap<String, String> communityPage = new FastMap<String, String>();
			htmlCode.setLength(0);
			StringUtil.append(htmlCode,
					"<html><body><br>"
					+ "<table><tr><td align=left valign=top>Server Restarted: ", String.valueOf(GameServer.dateTimeServerStarted.getTime()), "</td></tr></table>"
					+ "<table><tr>" + tdOpen + "XP Rate: x", String.valueOf(Config.RATE_XP), tdClose
					+ colSpacer + tdOpen + "Party XP Rate: x", String.valueOf(Config.RATE_XP * Config.RATE_PARTY_XP), tdClose
					+ colSpacer + tdOpen + "XP Exponent: ", String.valueOf(Config.ALT_GAME_EXPONENT_XP), tdClose + trClose
					+ trOpen + tdOpen + "SP Rate: x", String.valueOf(Config.RATE_SP), tdClose
					+ colSpacer + tdOpen + "Party SP Rate: x", String.valueOf(Config.RATE_SP * Config.RATE_PARTY_SP), tdClose
					+ colSpacer + tdOpen + "SP Exponent: ", String.valueOf(Config.ALT_GAME_EXPONENT_SP), tdClose + trClose
					+ trOpen
					+ tdOpen + "Drop Rate: ", String.valueOf(Config.RATE_DROP_ITEMS), tdClose
					+ colSpacer + tdOpen + "Spoil Rate: ", String.valueOf(Config.RATE_DROP_SPOIL), tdClose
					+ colSpacer + tdOpen + "Adena Rate: ", String.valueOf(Config.RATE_DROP_ITEMS_ID.get(57)), tdClose
					+ trClose
					+ "</table>"
					+ "<table>"
					+ trOpen + "<td><img src=\"sek.cbui355\" width=600 height=1><br></td>" + trClose
					...
					+ "</table>"
					+ "<table>" 
					+ trOpen + "<td>Color legend: <font color=\"LEVEL\">GM</font>, <font color=\"FFFF99\">Hero</font>, <font color=\"00FFFF\">Noble</font>, <font color=\"FF0000\">Karma</font>, <font color=\"00FF00\">Clan Leader</font>.</td> " + trClose
					+ "</table>");
			...
			if (Config.BBS_SHOW_PLAYERLIST)
			{
				...
				for (final L2PcInstance player : getOnlinePlayers(page))
				{
					...
					if (Config.COMMUNITY_COLOR_LEGEND)
					{
						if (player.isGM())
						{
							StringUtil.append(htmlCode, "<font color=\"LEVEL\">", player.getName(), "</font>");
						}
						else if (player.isInCraftMode() || player.isInStoreMode())
						{
							StringUtil.append(htmlCode, "<font color=\"CCCCCC\">", player.getName(), "</font>");
						}
						else if (player.isClanLeader()) //Clan Leader player, color green.
						{
							StringUtil.append(htmlCode, "<font color=\"00FF00\">", player.getName(), "</font>"); 
						}
						else if (player.getKarma() > 0) //PK Player, color red.
						{
							StringUtil.append(htmlCode, "<font color=\"FF0000\">", player.getName(), "</font>");
						}
						else if (player.isHero()) //Hero player, color yellow-white.
						{
							StringUtil.append(htmlCode, "<font color=\"FFFF99\">", player.getName(), "</font>");
						}
						else if (player.isNoble()) //Noble Player, color blue?
						{
							StringUtil.append(htmlCode, "<font color=\"00FFFF\">", player.getName(), "</font>");
						}
						else
						{
			...
			StringUtil.append(htmlCode,
					"<html><body><br>"
					+ "<table>"
					+ trOpen + "<td align=left valign=top>Server Restarted: ", String.valueOf(GameServer.dateTimeServerStarted.getTime()), tdClose + trClose
					+ "</table>"
					+ "<table>"
					+ trOpen + tdOpen + "XP Rate: ", String.valueOf(Config.RATE_XP), tdClose
					+ colSpacer	+ tdOpen + "Party XP Rate: ", String.valueOf(Config.RATE_PARTY_XP), tdClose
					+ colSpacer + tdOpen + "XP Exponent: ", String.valueOf(Config.ALT_GAME_EXPONENT_XP), tdClose + trClose
					+ trOpen + tdOpen + "SP Rate: ", String.valueOf(Config.RATE_SP), tdClose + colSpacer + tdOpen
					+ "Party SP Rate: ", String.valueOf(Config.RATE_PARTY_SP), tdClose
					+ colSpacer + tdOpen + "SP Exponent: ", String.valueOf(Config.ALT_GAME_EXPONENT_SP), tdClose + trClose
					+ trOpen + tdOpen + "Drop Rate: ", String.valueOf(Config.RATE_DROP_ITEMS), tdClose
					+ colSpacer + tdOpen + "Spoil Rate: ", String.valueOf(Config.RATE_DROP_SPOIL), tdClose
					+ colSpacer + tdOpen + "Adena Rate: ", String.valueOf(Config.RATE_DROP_ITEMS_ID.get(57)), tdClose
					+ trClose
					+ "</table>"
					...
					+ "<table>" 
					+ trOpen 
					+ "<td>Color legend: <font color=\"LEVEL\">GM</font>, <font color=\"FFFF99\">Hero</font>, <font color=\"00FFFF\">Noble</font>, <font color=\"FF0000\">Karma</font>, <font color=\"00FF00\">Clan Leader</font>.</td> " + trClose
					+ "</table>");
